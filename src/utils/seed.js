/**
 * Script de seed : à exécuter une seule fois après la mise en place du backend.
 * Crée les 7 métiers par défaut (avec leurs formulaires), le premier compte
 * admin, et les textes de base du site.
 *
...
async function seed() {
  ...
  console.log('--- Seed du contenu du site ---');
  ...
  if (!email || !password) {
    console.log('  ⚠ SEED_ADMIN_EMAIL / SEED_ADMIN_PASSWORD manquants dans .env — admin non créé.');
  } else {
    ...
    if (existing) {
      ...
      console.log(`  ✓ Admin créé : ${email} (changez ce mot de passe après connexion)`);
    }
  }
